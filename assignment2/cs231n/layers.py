...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        cache = (x, xhat, sample_mean, sample_var, eps, gamma, beta)
    elif mode == "test":
...
def batchnorm_backward(dout, cache):
    ...
    """
    x, xhat, mean, var, eps, gamma, beta = cache
    N, D = x.shape
    ...
    dvar = np.sum(dxhat * (x - mean) * -0.5 * (std_inv ** 3), axis=0)
    dmean = np.sum(dxhat * -std_inv, axis=0) + dvar * np.mean(-2.0 * (x - mean), axis=0)

    dx = dxhat * std_inv + dvar * 2.0 * (x - mean) / N + dmean / N
...
def layernorm_backward(dout, cache):
    ...
    dvar = np.sum(dxhat * (x - mean) * -0.5 * (std_inv ** 3), axis=1, keepdims=True)
    dmean = np.sum(dxhat * -std_inv, axis=1, keepdims=True) + dvar * np.mean(-2.0 * (x - mean), axis=1, keepdims=True)

    dx = dxhat * std_inv + dvar * 2.0 * (x - mean) / D + dmean / D
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    std = np.sqrt(var + eps)
    xhat_group = (x_group - mean) / std
    xhat = xhat_group.reshape(N, C, H, W)
    out = gamma * xhat + beta
    cache = (G, xhat_group, var, eps, gamma)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return out, cache
...
def spatial_groupnorm_backward(dout, cache):
    ...
    # This will be extremely similar to the layer norm implementation.        #
    ###########################################################################
    G, xhat_group, var, eps, gamma = cache
    N, C, H, W = dout.shape
    group_size = C // G
    M = group_size * H * W
    ...
    dxhat = dout * gamma
    dxhat_group = dxhat.reshape(N, G, group_size, H, W).reshape(N, G, M)
    xhat_group_flat = xhat_group.reshape(N, G, M)

    ivar = 1.0 / np.sqrt(var + eps)  # (N, G, 1, 1, 1)
    ivar = ivar.reshape(N, G, 1)
    dx_group = (1.0 / M) * ivar * (
        M * dxhat_group
        - np.sum(dxhat_group, axis=2, keepdims=True)
        - xhat_group_flat * np.sum(dxhat_group * xhat_group_flat, axis=2, keepdims=True)
    )
